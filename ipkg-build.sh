...
mkdir $tmp_dir/data
mkdir $tmp_dir/data/usr
mkdir $tmp_dir/data/usr/palm

cp -r $pkg_dir/src/* $tmp_dir/data/usr/palm/

cp $pkg_dir/$CONTROL/postinst $tmp_dir/pmPostInstall.script
cp $pkg_dir/$CONTROL/prerm $tmp_dir/pmPreRemove.script

tar -C $tmp_dir/data -czf $tmp_dir/data.tar.gz .
